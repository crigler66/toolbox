...
themes.addEventListener("click", (e) => {
  ...
  switch (targetTheme) {
    ...
    case "theme2":
      ...
      musicBox.style.boxShadow =
        "inset -10px -10px 15px #fffff0a, inset 10px 10px 15px #ffffff0a";
      blurElement.style.background =
        "linear-gradient(135deg, #dc143c, #0009688)";
    ...
    case "theme3":
      blurElement.style.visibility = "visible";
      musicBox.style.border = "1px solid #ffffff12";
      musicBox.style.boxShadow = "inset -10px -10px 15px #ffffff0a";
      blurElement.style.background =
...
nextButton.addEventListener("click", (e) => {
  let actvieAudio = document.querySelector(".active");

  let trackItems = document.querySelectorAll(".track-item");

  let activeIndex =
    +actvieAudio.dataset.index == trackItems.length - 1
      ? -1
      : +actvieAudio.dataset.index;

  let targetIndex = +activeIndex + 1;

  actvieAudio.classList.remove("active");
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.code == "ArrowDown" || e.code == "ArrowUp") {
    ...
    document.addEventListener("keyup", (e) => {
      ...
      document.addEventListener("keydown", (e) => {
        if (e.code == "ArrowDown" || e.code == "ArrowUp") {
          clearImmediate(volumeChangeAnimation);
        }
      });
    });
  }
